...
### create a folder named "03_output" if it doesn't exist 

if(!file.exists("./03_output")){dir.create("./03_output")}
...
list_FilePaths <- list.files(path='01_data/BWG_database',pattern="*.csv",full.names = TRUE)
...
write.csv(bromeliads_clean,'03_output/bromeliads_clean.csv')
write.csv(datasets_clean,'03_output/datasets_clean.csv')
write.csv(visits_clean,'03_output/visits_clean.csv')

### export sf object as GeoJSON for GIS
st_write(xy_datasets_utm, "03_output/xy_datasets_utm.geojson", driver = "GeoJSON",append=FALSE) # use append=FALSE to overwrite layer
st_write(xy_visits_utm, "03_output/xy_visits_utm.geojson", driver = "GeoJSON",append=FALSE)
